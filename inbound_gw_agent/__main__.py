...
async def _fetch_and_store_today_emails(pipeline: Pipeline) -> None:
    ...
    # 1순위: Microsoft Graph API (AZURE_CLIENT_ID 설정 시 활성화)
    if emails is None and settings.azure_client_id:
        try:
            ...
            log.warning("graph_email_fetch_failed", error=repr(exc)[:200])

    # 2순위: Classic Outlook COM (Graph API 미설정 시 fallback)
    if emails is None:
        try:
    ...
    for email in emails:
        ...
        msg_id = generate_message_id(sender, received_at, subject)
